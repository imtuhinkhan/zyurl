...
class User extends Model {
	...
	public function notifications(){
		if($notifications = \Core\DB::appevents()->whereRaw("type='notification' AND (userid IS NULL OR userid = '$this->id') AND (planid IS NULL OR planid = '".($this->planid ?? '0')."') AND (expires_at IS NULL OR DATE(expires_at) > DATE('".Helper::dtime()."'))")->orderByDesc('id')->findMany()){
